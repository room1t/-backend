...
public class OAuthServiceTest {
    ...
    public void loadExistGoogleUser() {
        ...
        User user = createUser (name, "화솔", email, provider);
        ...
        assertThat(providerObject.getName()).isEqualTo(registrationId);
    }
...
//        // given
//        OAuth2User oAuth2User = mock(OAuth2User.class);
//        String registrationId = OAuth2Provider.NAVER.getRegistrationId();
//        Provider provider = createProvider(registrationId);
//        // id null provider naver
//        Provider save = providerRepository.save(provider);
//        // pk가 생겨서 id 1 provider naver
//
//        String email = "hwasowl598@naver.com";
//        String name = "박정수";
//        User user = createUser(2L, name, "화솔", email, save);
//        userRepository.save(user);
//
//        OAuth2AccessToken accessToken = new OAuth2AccessToken(OAuth2AccessToken.TokenType.BEARER, "dummy-access-token", null, null);
//        ClientRegistration clientRegistration = createClientRegistration(registrationId);
...
//        assertThat(resultAttributes.get("name")).isEqualTo(name);
//        assertThat(resultAttributes.get("email")).isEqualTo(email);
//        assertThat(resultAttributes.get("provider")).isEqualTo(registrationId);
//    }
    ...
    private Provider createProvider(String registrationId) {
        return new Provider(registrationId);
    }

    private User createUser(String name, String nickname, String email, Provider provider) {
        return User.builder()
            .name(name)
            .nickname(nickname)
            .email(email)
